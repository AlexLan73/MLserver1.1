'''
class TimeWait - предназначен для инвертирования входного сигнала через N секунд
        count_sec   - время в секундах
        is_uprav    - входной сигнал инвертируется
        time_wait   - пересчитает счетчик на 1 секунду
        set(count_sec=0) - переустановит счетчик
            count_sec=0  - если 0 то по умолчанию
                         - если не 0 то по значению
'''
import threading
import time, copy
